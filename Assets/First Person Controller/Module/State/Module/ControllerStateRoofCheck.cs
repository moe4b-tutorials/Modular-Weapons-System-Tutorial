...
namespace FPC
{
	public class ControllerStateRoofCheck : ControllerState.Module
	{
        ...
        public float Radius => Controller.State.Radius.Value * 0.8f;
        public float Height => Controller.State.Height.Value;
        ...
        public virtual bool Active
        {
            get
            {
                ...
                var delta = Transition.Target.Height.Target - State.Height.Value;
        ...
        void Process()
        {
            if(Active)
            {
                if (Detect())
                {
                    var target = FindElement(State.Height.Value);
        ...
        protected virtual ControllerStateElement FindElement(float height)
        {
            for (int i = 0; i < Elements.Count; i++)
                if (Elements[i].Height.Target < height)
                    return Elements[i];
